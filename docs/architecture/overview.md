...
- If `LLM_API_KEY` is configured, the API asks the configured structured-output LLM endpoint for recommendation refinement using normalized findings, generated tests, evaluations, and baseline recommendations. Raw transcript turns are not sent to that adapter.
