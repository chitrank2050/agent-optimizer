  ...
  <section class="mx-auto grid max-w-7xl gap-6 px-6 py-8 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
    <div class="rounded-md border border-(--border) bg-white p-6">
      <div class="flex items-center gap-3">
        <Bot class="h-5 w-5 text-(--accent)" aria-hidden="true" />
      ...
      <dl class="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <dt class="text-sm font-medium text-(--muted)">Agents synced</dt>
          <dd class="mt-1 text-sm">{{ integration?.syncedAgents.length ?? 0 }}</dd>
        </div>
        <div>
          <dt class="text-sm font-medium text-(--muted)">Call logs found</dt>
          <dd class="mt-1 text-sm">{{ integration?.callLogs.length ?? 0 }}</dd>
        </div>
        <div>
          <dt class="text-sm font-medium text-(--muted)">Transcripts imported</dt>
          <dd class="mt-1 text-sm">{{ integration?.transcriptImports.imported ?? 0 }}</dd>
        </div>
      </dl>

      <button
        class="mt-6 inline-flex items-center gap-2 rounded-md bg-(--accent) px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
        type="button"
        :disabled="isSyncing"
        @click="$emit('sync')"
      >
