...
| AI Core   | TypeScript + LLM adapter      | Stable deterministic baseline with optional structured-output refinement      |
...
- Optional LLM adapter that refines recommendations with structured JSON output when `LLM_API_KEY` is configured
...
- LLM recommendation refinement is optional and sends normalized findings/tests/evaluations, not raw transcript turns
